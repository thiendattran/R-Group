...
a string is sorted by anaphabet
...
levels(x)
```
 pardéfaut, fct_reorder2() utilise function last2 (trier les valeurs de 'prop' à la dernière groupe d'âge(89 ans)
 first2: trier les valeurs de 'prop' à la première groupe d'âge(18 ans)
 labs() = labels
```{r}
by_age <- gss_cat %>%
  filter(!is.na(age)) %>%
...
fct_rev() renverse order of factor levels
